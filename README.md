...
| `wizard` | Auto-available | Build guided Bash flows for manual setup, secrets, migrations, or cutovers. | Manual procedure and destinations for captured values | Bundled `template.sh`; relevant service documentation when UI steps are uncertain | Human runs the generated wizard |
